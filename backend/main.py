...
def get_year_list(years_ahead=3):
    current_year = datetime.now().year
    return list(range(current_year, current_year + years_ahead + 1))
...
def create_model(data, column):
    if column not in model_cache:
        model = Prophet(
            ...
            holidays=make_holidays_df(year_list=get_year_list(), country='ID')
        )
...
def create_model_for_mape(data, column):
    if column not in model_cache:
        model = Prophet(
            yearly_seasonality=True,
            weekly_seasonality=True,
...
def get_all_mape():
    ...
    for pollutant in pollutants:
        try:
            model = create_model_for_mape(df, pollutant)
...
# Jalankan dengan uvicorn jika perlu
